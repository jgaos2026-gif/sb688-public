...
export class FileUploadManager {
  ...
  private static readonly MAX_STORE_FILES = 200;
  ...
  receive(request: UploadRequest): UploadResult {
    ...
    if (this.store.size >= FileUploadManager.MAX_STORE_FILES) {
      const storeFullAnomaly = ["store_full"];
      this.appendLog("receive", filename, "rejected", scan.contentHash, at, {
        anomalies: storeFullAnomaly
      });
      this.ledger.append({
        traceId: `upload:${filename}`,
        from: "intent",
        ...
        status: "failed",
        at,
        detail: { action: "receive_upload", filename, anomalies: storeFullAnomaly, contentHash: scan.contentHash }
      });
      return {
        accepted: false,
        filename,
  ...
  dispatch(request: DispatchRequest): DispatchResult {
    ...
    if (destination.includes("..") || destination.includes("/") || destination.includes("\\")) {
