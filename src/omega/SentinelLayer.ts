...
import type {
  DriftReport,
  OmegaSentinelDiagnosis,
  OmegaSentinelHealthMetric,
  OmegaSentinelRecommendation,
  OmegaSentinelStatus
} from "./contracts";
...
export class SentinelLayer {
  ...
  private readonly metrics: OmegaSentinelHealthMetric[] = [];
  ...
  observe(tick: number, report: DriftReport): void {
    const metric: OmegaSentinelHealthMetric = Object.freeze({
      tick,
      at: this.clock(),
      drift: report.drift,
  ...
  diagnose(): OmegaSentinelDiagnosis {
    ...
    if (windowSize === 0) {
      return Object.freeze({
        ...
        recommendation: "NOMINAL" as OmegaSentinelRecommendation,
  ...
  private _explain(
    rec: OmegaSentinelRecommendation,
    breachRate: number,
    consecutiveBreaches: number
  ): string {
    switch (rec) {
