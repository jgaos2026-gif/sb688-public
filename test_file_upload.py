...
def test_dispatch_rejects_path_separator_in_destination():
    spine, bricks, sentinel, mgr = make_stack()
    mgr.receive("report.json", b'{"v":2}', "application/json")
    for bad in ["invoices/2026", "a\\\\b", "sub/dir"]:
        ok, result = mgr.dispatch("report.json", bad)
        assert ok is False, f"Expected rejection for destination={bad!r}"
        assert result["reason"] == "invalid_destination"
