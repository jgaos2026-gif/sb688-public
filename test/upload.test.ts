...
test("FileUploadManager refuses dispatch to destination with path separators", () => {
  const ledger = new AuditLedger();
  const mgr = new FileUploadManager(ledger, undefined, fixedClock(AT));

  mgr.receive({ filename: "report.json", content: '{"v":2}', contentType: "application/json" });

  for (const bad of ["invoices/2026", "a\\b", "sub/dir"]) {
    const result = mgr.dispatch({ filename: "report.json", destination: bad });
    assert.equal(result.dispatched, false, `Expected rejection for destination=${bad}`);
  }
});
